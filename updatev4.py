...
def build_report_markdown(target_date, report_data):
    ...
    if task_unupdated_records:
        ...
        lines.append("说明：只要任务下存在任一 owner 三项未完整填写，任务会进入本表；任务名称已去除【X次未更新】/【X日未更新】/【格式错误】/【回写失败】标签。")
        lines.append("")
        lines.append("| 任务Code | 任务名称 | 未更新次数 | 未完整更新负责人 |")
        lines.append("| --- | --- | --- | --- |")

        for record in task_unupdated_records:
            todo_code = escape_md_cell(record.get("todo_code", ""))
            task_title = escape_md_cell(strip_title_prefixes(record.get("task_title", "")))
            days = escape_md_cell(str(record.get("days_not_updated", "")))
            owners = "、".join(record.get("owners", {}).values())

            lines.append(f"| {todo_code} | {task_title} | {days}次 | {escape_md_cell(owners)} |")

        lines.append("")

    if unupdated_records:
        lines.append("## 个人未更新情况")
        lines.append("说明：个人未更新只统计已在日报中解析到的 owner-task；不会因为同任务其他 owner 未填写而误报任务缺失。")
        lines.append("")
        ...
        for r in unupdated_records:
            ...
            missing = escape_md_cell(r.get("missing_fields", ""))
            days = escape_md_cell(str(r.get("days_not_updated", "")))

            lines.append(f"| {owner} | {dept} | {code} | {title} | {missing} | {days}次 |")
    ...
    if format_error_records:
        ...
        for r in format_error_records:
            ...
            code = escape_md_cell(r.get("todo_code", ""))
            title = escape_md_cell(strip_title_prefixes(r.get("task_title", "")))
            field = escape_md_cell(r.get("error_field", ""))
            raw = escape_md_cell(r.get("error_raw", ""))
            expected = escape_md_cell(r.get("error_expected", ""))

            lines.append(f"| {owner} | {dept} | {code} | {title} | {field} | {raw} | {expected} |")

        lines.append("")

    if failed_records:
        lines.append("## 异常/缺失记录")
        lines.append("说明：这里仅统计任务级缺失或接口异常；不会把某个 owner 未出现误判成任务缺失。")
        lines.append("")
...
for row in mapping_rows:
    ...
    for tid, group_tasks in tasks_by_todo_id.items():
        ...
        # A2. 多 owner 缺席补统计：
        #     任务已经在日报中出现，不算异常/缺失；
        #     但 TodoList 中某些 owner 没有填写该任务时，计入个人未更新。
        # ------------------------------------------------------------
        if source_todo:
            ...
            if missing_owner_records:
                print(
                    f"ℹ️ 多 owner 缺席统计: todo={tid}, "
                    f"missing_owner_count={len(missing_owner_records)}"
                )
                unupdated_records.extend(missing_owner_records)
