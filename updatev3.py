...
def build_report_markdown(target_date, report_data):
    ...
    task_unupdated_records = build_task_unupdated_summary(unupdated_records)

    lines = []

    lines.append("<h2>任务更新情况汇总</h2>")
    lines.append(f"<p>总任务数：<strong>{total_tasks}</strong>，涉及 owner 总数：<strong>{total_owners}</strong></p>")
    lines.append(f"<p>成功更新：<strong>{len(success_records)}</strong> 条记录，<strong>{count_unique_owners(success_records)}</strong> 人</p>")
    lines.append(f"<p>未更新：<strong>{len(unupdated_records)}</strong> 条记录，<strong>{count_unique_owners(unupdated_records)}</strong> 人</p>")
    lines.append(f"<p>格式错误：<strong>{len(format_error_records)}</strong> 条记录，<strong>{count_unique_owners(format_error_records)}</strong> 人</p>")
    lines.append(f"<p>异常/缺失记录：<strong>{len(failed_records)}</strong> 条，涉及 <strong>{count_unique_owners(failed_records)}</strong> 人</p>")
    lines.append("")

    if task_unupdated_records:
        lines.append("<h2>任务未更新状态汇总</h2>")
        lines.append("<p>说明：只要任务下存在任一 owner 三项未完整填写，任务会进入本表；任务名称已去除【X次未更新】/【格式错误】/【回写失败】标签。</p>")
        lines.append("<table><thead><tr><th>任务Code</th><th>任务名称</th><th>未更新次数</th><th>未完整更新负责人</th></tr></thead><tbody>")

        for record in task_unupdated_records:
            todo_code = escape_html(record.get("todo_code", ""))
            task_title = escape_html(strip_title_prefixes(record.get("task_title", "")))
            days = escape_html(str(record.get("days_not_updated", "")))
            owners = "、".join(record.get("owners", {}).values())

            lines.append(
                f"<tr><td>{todo_code}</td><td>{task_title}</td>"
                f"<td>{days}次</td><td>{escape_html(owners)}</td></tr>"
            )
    ...
    if unupdated_records:
        ...
        lines.append("<p>说明：个人未更新只统计已在日报中解析到的 owner-task；不会因为同任务其他 owner 未填写而误报“缺少进展描述、缺少承诺完成时间、缺少风险等级”。</p>")
    ...
    if format_error_records:
        ...
        lines.append("<table><thead><tr><th>负责人</th><th>部门</th><th>任务Code</th><th>任务名称</th><th>错误字段</th><th>填写值</th><th>正确格式</th></tr></thead><tbody>")
        ...
        for r in format_error_records:
            ...
            lines.append(
                f"<tr><td>{owner}</td><td>{dept}</td><td>{code}</td>"
                f"<td>{title}</td><td>{field}</td><td>{raw}</td><td>{expected}</td></tr>"
            )
    ...
    if failed_records:
        lines.append("<h2>异常/缺失记录</h2>")
        lines.append("<p>说明：这里仅统计任务级缺失或接口异常；不会把某个 owner 未出现误判成任务缺失。</p>")
...
def build_report_card_content(report_data):
    ...
    lines.append(f"**成功更新**：{len(success_records)} 条记录，{count_unique_owners(success_records)} 人")
    lines.append(f"**未更新**：{len(unupdated_records)} 条记录，{count_unique_owners(unupdated_records)} 人")
    lines.append(f"**格式错误**：{len(format_error_records)} 条记录，{count_unique_owners(format_error_records)} 人")
    lines.append(f"**异常/缺失记录**：{len(failed_records)} 条，涉及 {count_unique_owners(failed_records)} 人")

    if unupdated_records:
        names = sorted({
            f"{r.get('owner_name', '')}({r.get('owner_dept', '')})" if r.get("owner_dept") else r.get("owner_name", "")
            for r in unupdated_records
            if r.get("owner_name") or r.get("owner_dept")
        })
        lines.append("")
        lines.append("**未更新人员**：" + "、".join(names[:20]))
    ...
    if format_error_records:
        ...
        lines.append("")
        lines.append("**格式错误人员**：" + "、".join(names[:20]))
    ...
    if failed_records:
        ...
        lines.append("")
        lines.append("**异常/缺失涉及人员**：" + "、".join(names[:20]))
        ...
        for task in enriched_tasks:
            ...
            if status_info["is_unupdated"]:
                unupdated_records.append({
                    ...
                    "owner_dept": owner_dept,
                    "todo_code": task.get("todo_code", ""),
                    "task_title": strip_title_prefixes(real_title),
